  ...
  if (loading && workflows.length === 0) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        ...
        >
          <div className="w-16 h-16 border-4 border-purple-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-xl text-white">Loading workflows...</p>
        </motion.div>
      </div>
    );
  }
    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-slate-900' : 'bg-gray-50'}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => setDarkMode(!darkMode)}
                className={`p-2 rounded-lg ${darkMode ? 'bg-slate-700 text-yellow-400' : 'bg-gray-200 text-purple-600'}`}
              >
                {darkMode ? '☀️' : '🌙'}
              </motion.button>
              ...
              <div className="relative">
                <motion.button
                  ...
                  disabled={exporting}
                  className="bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-2 rounded-lg font-semibold shadow-lg disabled:opacity-50 flex items-center space-x-2"
                >
                  ...
                  {showExportMenu && (
                    ...
                    >
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 hover:bg-slate-700 rounded-t-lg flex items-center space-x-2 ${
                      ...
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 hover:bg-slate-700 rounded-b-lg flex items-center space-x-2 ${
                          darkMode ? 'text-white' : 'text-gray-900'
                        }`}
                      >
                        <span>📝</span>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        >
          ...
          <p className={`text-xl ${darkMode ? 'text-gray-300' : 'text-gray-600'} mb-6`}>
            Real-time data from YouTube, Forum, Search & GitHub
          </p>
          <div className="flex items-center justify-center space-x-2">
            <motion.div
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
            {[
              { icon: '📊', label: 'Total', value: stats.total_workflows, color: 'purple' },
              { icon: '🎥', label: 'YouTube', value: stats.platforms?.YouTube?.count || 0, color: 'red' },
              { icon: '💬', label: 'Forum', value: stats.platforms?.['n8n Forum']?.count || 0, color: 'green' },
              { icon: '🐙', label: 'GitHub', value: stats.platforms?.GitHub?.count || 0, color: 'purple' },
            ].map((stat, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
        ...
        >
            ...
            <div className="md:col-span-2 relative">
              ...
              <input
                ...
                className={`w-full pl-12 pr-4 py-3 rounded-lg border focus:outline-none focus:ring-2 focus:ring-purple-500 ${
                  darkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-gray-50 border-gray-300'
                }`}
              />
            </div>
          ...
          <div className="flex items-center space-x-3">
            <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Sort:</span>
            {['engagement', 'views', 'likes'].map((sort) => (
              <motion.button
                key={sort}
                whileHover={{ scale: 1.05 }}
          ...
          {filteredWorkflows.slice(0, 50).map((workflow, index) => (
            <motion.div
              ...
              className={`rounded-2xl p-6 shadow-lg ${
                darkMode ? 'bg-slate-800/50 border border-slate-700 hover:border-purple-500' : 'bg-white border border-gray-200 hover:border-purple-400'
              }`}
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-3">
                    <motion.span
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${getPlatformColor(workflow.platform)} text-white`}
                    >
                      {getPlatformIcon(workflow.platform)} {workflow.platform}
                    </motion.span>
                    <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      🌍 {workflow.country}
                    </span>
                ...
                <div className="text-right">
                  <motion.div
                    whileHover={{ scale: 1.1 }}
                    className="text-4xl font-bold bg-gradient-to-r from-purple-500 to-indigo-600 bg-clip-text text-transparent"
                  >
                    {workflow.popularity_metrics.engagement_score.toFixed(1)}
                  </motion.div>
                  <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {workflow.popularity_metrics.views > 0 && (
                  <motion.div whileHover={{ scale: 1.05 }} className={`rounded-lg p-3 ${darkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span>👁️</span>
                      <span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {formatNumber(workflow.popularity_metrics.views)}
                    ...
                    <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Views</div>
                  </motion.div>
                )}
                ...
                {workflow.popularity_metrics.likes > 0 && (
                  <motion.div whileHover={{ scale: 1.05 }} className={`rounded-lg p-3 ${darkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                    ...
                    <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Likes</div>
                  </motion.div>
                )}
                ...
                {workflow.popularity_metrics.comments > 0 && (
                  <motion.div whileHover={{ scale: 1.05 }} className={`rounded-lg p-3 ${darkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                    ...
                    <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Comments</div>
                  </motion.div>
                )}
                ...
                {workflow.popularity_metrics.search_volume > 0 && (
                  <motion.div whileHover={{ scale: 1.05 }} className={`rounded-lg p-3 ${darkMode ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                    ...
                    <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Searches</div>
                  </motion.div>
                )}
              </div>
            </motion.div>
        ...
        {filteredWorkflows.length === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`rounded-2xl p-12 text-center ${darkMode ? 'bg-slate-800/50' : 'bg-white'}`}
          >
            <div className="text-6xl mb-4">🔍</div>
            <h3 className={`text-2xl font-semibold mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              No workflows found
      ...
      {/* Footer */}
      <footer className={`mt-20 py-12 border-t ${darkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-white/50 border-gray-200'}`}>
        <div className="max-w-7xl mx-auto px-6 text-center">
          <p className={`mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            Built for SpeakGenie Internship • 4 Data Sources • 50+ Workflows
          </p>
          <p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
            YouTube • n8n Forum • Google Search • GitHub
          </p>
        </div>
      </footer>
    </div>
